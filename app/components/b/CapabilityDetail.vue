  ...
  <div class="space-y-6">
    ...
    <div v-if="relatedPromos.length" class="flex gap-3">
      ...
      >
        ...
        <NuxtLink to="/b/promotions" class="text-white/80 text-xs underline hover:text-white">了解详情</NuxtLink>
      </div>
    </div>
      ...
      <div v-if="activeTab === 'overview'" class="grid grid-cols-3 gap-6">
        ...
        <div class="space-y-4">
          ...
          <div v-if="isModel && model" class="bg-white rounded-xl border border-gray-100 p-5 space-y-4">
            ...
            <UButton label="立即体验" icon="i-lucide-play" color="primary" block size="sm" />
          </div>
